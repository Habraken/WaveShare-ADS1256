...
#define LED_PIN         27      // Offboard LED pin
...
int main(int argc, char *argv[])
{
    ...
    gpio_out(LED_PIN, 1); // Ensure we leave the led on...
    terminate(0);
}
...
typedef struct {
    DMA_CB cbs[NUM_CBS];
    uint32_t pindata, pwmdata1, pwmdata2;
} DMA_TEST_DATA;
...
void dma_test_pwm_trigger(int pin)
{
    ...
    DMA_TEST_DATA dma_data = {
        .pindata=1<<pin, .pwmdata1=PWM_RANGE/2, .pwmdata2=PWM_RANGE/4,
        .cbs = {
          // TI      Srce addr          Dest addr        Len   Next CB
            {PWM_TI, MEM(&dp->pindata), GPIO(GPIO_CLR0), 4, 0, CBS(1), 0},  // 0
            {PWM_TI, MEM(&dp->pwmdata1), PWM(PWM_RNG1),  4, 0, CBS(2), 0},  // 1
            {PWM_TI, MEM(&dp->pwmdata1), PWM(PWM_FIF1),  4, 0, CBS(3), 0},  // 2
            {PWM_TI, MEM(&dp->pindata), GPIO(GPIO_SET0), 4, 0, CBS(4), 0},  // 3
            {PWM_TI, MEM(&dp->pwmdata2), PWM(PWM_RNG1),  4, 0, CBS(5), 0},  // 4
            {PWM_TI, MEM(&dp->pwmdata2), PWM(PWM_FIF1),  4, 0, CBS(0), 0},  // 5
        }
    };
